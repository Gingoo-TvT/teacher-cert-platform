...
## 10. 验收清单（AT-01/02/14）
- [ ] 导出标准表含 A–Z **26 列**，名称顺序与模板一致，**H 列表头="身份证件号码"**（AT-02）。
- [ ] 学校代码/学号/证件号/出生日期/证书编号/有效期限文本导出；重开 Excel **无科学计数、无日期序列、无前导零丢失**（AT-01）。
- [ ] 模板下拉项与系统字典一致；全列文本格式。
- [ ] V-01~V-13 **逐条**可触发，定位行号/字段/原因（13 条各一反例）。
- [ ] 异常数据不静默入库；异常报告含行号/字段/错误值/原因/建议（AT-14）。
- [ ] 4 种导入策略行为正确；批次可查；回滚后恢复到导入前。
- [ ] 5 类导出列与 §15.6 一致；导出留审计；敏感导出鉴权。

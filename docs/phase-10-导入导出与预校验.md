...
WS-8 存储边界：导入预览 `preview_json/scope_json`、错误明细中的身份证件号码错误值，以及
`import_record_ref.before_json/after_json` 的身份证件号码均不得持久化为可读明文；业务投影按既有权限解密后
脱敏或返回。回滚比较使用 HMAC 规范值，不依赖随机密文相等。

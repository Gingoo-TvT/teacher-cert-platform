...
## 9. 验收清单
- [x] 四类证件分别用合法/非法号码 → 合法放行、非法拦截并提示"证件类型与号码不匹配"（AT-03）。
- [x] 身份证号第 7–14 位与出生日期不一致 → 拦截提示（AT-03）。
- [x] 港澳通行证/台胞证**不做**出生日期一致校验（按 §15.4）。
- [x] 姓名含数字/字母/空格/非法符号 → 拦截；含"·"的少数民族姓名在对应模式下放行。
- [x] 列表与详情默认脱敏（前6后4）；无 `export:sensitive` 取明文被拒并无明文泄漏。
- [x] 证件号/学号/出生日期落库为文本，回显与录入完全一致（无前导零丢失、无日期序列）（AT-01 部分）。
- [x] WS-8 本地阶段候选：`student.id_card_no` 与证书快照使用独立随机 IV 的 AES-GCM 密文落库，
  `id_card_hmac` 只用于等值查重/唯一约束；普通投影只返回脱敏值，既有敏感权限接口解密后留审计。
- [x] 两级审核状态流转与留痕正确；退回必填原因；保留历史版本。
- [x] 复审通过/证书生成后关键字段锁定，学生与教务员均禁改（特批除外，留痕）。
- [x] 开通的学生账号可用学号登录，首次强制改密。
- [x] Phase 39 PG-H1 整改：学生单条新增、批量新增和学院迁移均先取得 RBAC 授权锁，再锁目标学院父行；批量跨学院先完成数据范围校验，再按学院 ID 去重升序锁定。学院删除同时直接统计 `student`，不再依赖自动开户产生的 `sys_user` 作为间接守卫。
...
- T-ID-PROTECT-1：同一合法证件号写入学生与证书 → 两份密文不同、均可用原密钥解密，HMAC 相同；
  重复活跃学生仍被唯一约束拒绝，逻辑删除同时释放 HMAC 唯一键。
- T-ID-PROTECT-2（反例）：缺失/示例密钥或篡改 `v1:` 密文 → 启动/迁移或读取失败关闭，日志不包含明文证件号。
- T-LOCK-1：证书生成后改证件号 → 拒。
- T-SM-1：待初审→初审不通过 → 状态=不合格(终止)，再次提交被拒（需特批重置）。
- T-COLLEGE-LOCK-1（反例）：`student.autoCreateAccount=false` 时学生新增先锁父行 → 学院删除等待后因活跃学生拒绝；无旁生 `sys_user`，孤儿数为 0。✅ `Phase39CollegeIntegrityIT`。
- T-COLLEGE-LOCK-2（反例）：学院删除先提交 → 学生新增在父锁定读后以“学院不存在”失败；无学生、无账号、无孤儿。✅ `Phase39CollegeIntegrityIT`。

...
## 5. 验收清单
- [ ] 一键 `docker compose up` 起站，可登录、可跑通主流程。
- [ ] M14 开关关闭时一期功能不受影响；开启时不报错（占位）。
- [ ] **AT-01~AT-14 整体复验全部通过**（用 docs 各 Phase 自测用例汇总）。
- [ ] 导出文件在 Microsoft Excel 与 WPS 打开格式一致、文本字段不被转换（AT-01）。
- [ ] 主流程 E2E 贯通：导入→确认→培养→材料→初复审→免考→视频→测试→教务处确认→证书生成→签发→导出/归档。
- [ ] 主流浏览器（Chrome/Edge/Firefox）回归通过。

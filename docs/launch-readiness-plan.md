...
| P0-6 | ~~**备份是"演练记录"非真备份**：triggerBackup 仅插一行 COMPLETED；全库 0 处 `@Scheduled`；运维手册命令指向 dev 容器/密码~~ | ~~`SystemManagementServiceImpl.java:132-146`~~ | — | **已实现待复核合并（Phase 41.2，见 §11）：`DatabaseBackupService` JDBC 逻辑导出→gzip→MinIO(`db-backup/`) 真实产物 + `backup_record` 关联真实 size/checksum/表数/行数 + RUNNING→COMPLETED/FAILED；`BackupScheduleConfig` 定时(prod 门禁)；手册改指 prod 栈（迁移 V25）** | 待复核合并 |
...
- ✅ **往返列复用坏账**：导出把 `status` 写入"备注"列（`:865`），导入把该列当学院id `parseLong`（`:1058`）→ **重导入导出文件必坏**（学院识别失败）。**已修复（Phase 43.2，待复核合并，见 §11）：** `rowFromCertificate` 备注列改写「学院ID」（`cert.getCollegeId()`），与导入端 `resolveCollegeId` 读备注解析学院ID 对齐 → 导出→导入 学院标识无损往返；证书状态另由汇总表/完整审核表的「证书状态」专列承载，不再复用备注一列表达两种含义（模板 26 列 A-Z 布局未变，仅修正自由「备注」列内部语义）。IT 复现→阻断（导出备注=学院ID 非"ISSUED"、读回重导入仍落原学院）。
- ✅ **导入 cert_no 不占 `cert_sequence`** → 与自动生成永久撞号，`generate` 撞号回滚又不推进序列 → "证书编号已存在请重试"**永远失败**，需手工改库。**已修复（Phase 43.2，待复核合并，见 §11）：** 导入证书落库后调 `CertificateService.reserveImportedSequence(certNo)`，按与 `nextCertNo` 同一 scopeKey 规则把序列推进到 ≥导入号序号 → 后续 `generate` 跳过已占号。IT 端到端：导入占号 00001 后 generate 得 00002（非撞号死锁）。
- ✅ **导入证书 status=ISSUED 但 issueDate 从不设**（`applyCertificate` 只设 issuer/validUntil），且无补设路径（issue 要 GENERATED、correct 无 issueDate 字段）。**已修复（Phase 43.2，待复核合并，见 §11）：** `applyCertificate` 对「已签发且签发日期为空」的导入证书补一个与 `validUntil` 规则自洽的签发日期（证书年度 + 有效期上/下半年，对其再套 validUntil 规则可复现导入有效期），既覆盖新导入亦补设既有缺失（补设路径）。
...
### 7.7 校验 fail-open P2
- `StudentStatus/TrainingStatus/MaterialStatus.of` 未知值 `orElse(DRAFT)` → 损坏状态静默变最可编辑草稿（fail-open）；`ExemptionStatus.of` 却 `orElseThrow`（不一致）。**✅ 已修复待复核合并（Phase 43.4，见 §11）：** 三者改 `orElseThrow`（`BizException`），与 `ExemptionStatus.of` 一致；改前对 `student`/`training_profile`/`process_material` 三表（含软删行）做 `SELECT DISTINCT status` 安全预检，确认无脏值方切换。
- `BirthDateValidator` 只校验月 1-12 日 1-31 → 接受 2023-02-30。**✅ 已修复待复核合并（Phase 43.4，见 §11）：** 改用 `LocalDate.of(year,month,day)` 日历规则校验（含闰年/大小月），2023-02-30 等日历上不存在的日期现被拒绝。
...
### 7.11 部署 · 运维 · 依赖 · TLS
- **P0 Spring Boot 3.2.11 已 EOL + 命中方法级鉴权 CVE 影响面**（`pom.xml:10`）：3.2.x 2024-11 停 OSS 支持、2025-12 全面 EOL；传递 Spring Security 6.2.x，2025 年鉴权绕过族（CVE-2025-41249/41248/41232）正打 `@EnableMethodSecurity`+`@PreAuthorize`——**本平台 RBAC 的根基**。配置无法缓解，**上线前须升到受支持的 3.3+/3.4+**。**✅ 已修复（Phase 40，待复核合并，见 §11）：** 父 POM 升 **Spring Boot 3.2.11 → 3.4.13**（传递 **Spring Security 6.2.x → 6.4.x**，含上述 CVE 族修复）；同步 **MyBatis-Plus 3.5.7 → 3.5.16**（+ 显式 `mybatis-plus-jsqlparser-4.9`）、**knife4j 保持 4.5.0** 但用 `springdoc-openapi-bom 2.8.17` 覆盖其传递的 2.3.0（否则 SB3.4 的 Spring Framework 6.2 下 doc.html `NoSuchMethodError`）。`mvn verify` **93/93 绿**、Flyway 9.22→10.20 平滑（25 迁移校验通过、库仍 V25），前端零改动 type-check/build 绿；**RBAC 活体 3 条通过**（学生 `GET /system/user`→403、学院A clerk 读学院B学生→未找到、本域→200），证方法级安全 + 数据范围未因升级被绕过。
...
2. **P0-8 Spring Boot EOL + 方法鉴权 CVE**（§7.11）——升级 3.3+/3.4，动摇整个 RBAC。**✅ 已修复（Phase 40，待复核合并，见 §11）：升 SB 3.4.13 / Security 6.4.x，mvn verify 93/93、RBAC 活体 3 条通过。**
...
8. Spring Boot 3.2.11 EOL + 方法鉴权 CVE（§7.11）— 升 3.3+/3.4。**✅ 已修复待复核合并（Phase 40，见 §11）：升 SB 3.4.13（Security 6.4.x，含 CVE 修复）+ MP 3.5.16 + knife4j 4.5.0/springdoc 2.8.17；mvn verify 93/93 绿、RBAC 活体 3 条通过（未合并入 main）。**
...
### Phase 41.2（P0-6 真备份，Phase 41.2，待复核合并 —— 分支 `feature/phase41-backup`，单 commit，未合并入 main，mvn verify 91/91 绿；栈起 fresh jar 活体复现→阻断，迁移 V25）
...
### Phase 42.2（Phase 42.2，待复核合并 —— 分支 `feature/phase42-confirmimport`，单 commit，未合并入 main，mvn verify 84/84 绿；真栈并发 IT 复现→阻断）
...
### Phase 42.3（Phase 42.3，待复核合并 —— 分支 `feature/phase42-video`，单 commit，未合并入 main，mvn verify 86/86 绿；真栈并发 IT 复现→阻断）
...
### Phase 42.1（Phase 42.1，待复核合并 —— 分支 `feature/phase42-uniques`，单 commit，未合并入 main，mvn verify 83/83 绿；栈起 2 并发活体复现→阻断）
...
### Phase 43.4（Phase 43.4，待复核合并 —— 分支 `feature/phase43-failopen-dict`，单 commit，未合并入 main，mvn verify 88/88 绿）
...
### Phase 43.2（Phase 43.2，待复核合并 —— 分支 `feature/phase43-cert-roundtrip`，单 commit，未合并入 main，mvn verify 90/90 绿；导出→导入往返/序列/签发日期 IT 复现→阻断）
...
### Phase 43.3（Phase 43.3，待复核合并 —— 分支 `feature/phase43-rule11`，单 commit，未合并入 main，mvn verify 93/93 绿；真栈 IT 复现→阻断 + fresh jar 活体演示恢复路径）
...
### Phase 40（P0-8 Spring Boot EOL + 方法鉴权 CVE 升级，Phase 40，待复核合并 —— 分支 `feature/phase40-springboot`，单 commit，未合并入 main，mvn verify 93/93 绿；栈起 fresh jar RBAC 活体回归）

...
- **缺唯一约束致并发重复**：证书 `(student_id, assessment_year)` 仅非唯一索引（`V15:31-36`）→ 并发生成两张有效证书【✅ Phase 42.1，待复核合并：V24 生成列 `active_key` + `uk_cert_active`】；`student.id_card_no` 非唯一（`V9:31`）→ 并发建同证件号两人【✅ Phase 42.1，待复核合并：V24 生成列 `idcard_key` + `uk_student_idcard`】；`training_profile` 导入无唯一码 → confirmImport 双确认产生重复培养行【⏳ Phase 42.2】。

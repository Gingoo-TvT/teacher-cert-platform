...
## 6. 鉴权播放与水印
- 播放地址=登录绑定的应用代理，`video:play` 鉴权后下发短时 HttpOnly 媒体 Cookie；
  未登录、越权或退出后的复制内容链接均不可访问。
...
## 8. 验收清单（AT-08）
> 2026-07-24 WS-3 第六轮独立复核 **PASS**，确认第五轮新增的 scheduler trigger 隔离与 V32 candidate 全量备份覆盖 **2 Medium 全部关闭**，见 `reviews/ws-03-sixth-remediation-rereview-2026-07-24.md`。唯一新增 Low 是提交材料把 32 个迁移误写成 33 个，不阻断；T-VID-2L/2M 的开发者动态证据保留。下列前两项仍因真实 2GB 传输、非允许编码与不可解码首帧证据债保持未完成，播放项则因当前应用取流合同尚无新鲜动态证据而保持未完成，均不能由本轮小样本替代。
...
- T-VID-8（反例）：取得内容地址后清除登录态或退出，再直接播放 → 失败。

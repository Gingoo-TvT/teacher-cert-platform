...
- `logs/envA-run1-demo-on.log.txt`：四个内容版本对象逐一「已新建并验证」，随后「内容版本对象新建 4、一致跳过 0，实体 SQL 执行=成功」。
...
- `logs/envA-run2-demo-on-idempotent.log.txt`：「新建 0、一致跳过 4，实体 SQL 执行=成功」。
- `07/08/09`：DB 快照与 run1 逐字段 diff 为空；8 个对象 Name/Size/ETag diff 为空，对象 Last-Modified 仍为各自首次写入时刻（未重传）。

### 3.4 预存污染拒绝（步骤 4，环境 B，两变体）
- 变体一（同长度错内容）：向 material 版本 key 预存 659B 随机内容（SHA-256 `9a8debeb…`，Content-Type 正确，`10`）。demo 启动：视频对象正常新建后，在 material 处失败关闭，进程退出、无 8080 监听（`logs/envB-run1-demo-on-polluted-content.log.txt`，仅输出异常类别）。`11`：污染对象 ETag/Date/读回 SHA-256 与注入时完全一致（未被覆盖也未被“修复”）；**全部 demo 表 0 行（实体 SQL 未执行）**；仅遗留无引用视频版本对象（设计允许）。
...
- demo 启动：四个版本对象新建成功后，实体脚本失败（`ScriptStatementFailedException` 类别），启动终止（`logs/envC-run1-demo-on-sql-fault.log.txt`）。
...
- DROP 触发器后两次启动均「新建 0、一致跳过 4，实体 SQL 执行=成功」（`logs/envC-run2-demo-on-fault-cleared.log.txt`/`logs/envC-run3-demo-on-restart.log.txt`）；`17`：旧 key 残留引用 0、`file_object` 8 行、`video_review` 3 行均 900 秒——与环境 A 终态同构，行数稳定无重复。
...
## 6. 2026-07-26 证据整改附录（对应 `phase-53-remediation-rereview-2026-07-26.md` 的证据项 finding）

1. **Medium（12 个启动日志未提交）**：12 个原始 `.log` 经敏感模式扫描（password/passwd/secret/minioadmin/root123/X-Amz-Signature/Authorization/Bearer/eyJhb/ChangeMe/Phase53!Verify）零命中，无需内容脱敏，以**逐字节相同**的 `.log.txt` 归档于 `logs/`（`*.log.txt` 不命中 `.gitignore:7` 的 `*.log` 规则，原始 `.log` 仍按忽略规则留存本机）。`.gitattributes` 为 `logs/*.log.txt` 设 `-text -diff`（与 phase41 证据日志同口径），保证入库 blob 与原始字节一致、不因换行归一化漂移。`logs/logs-sha256-manifest.txt` 记录每个文件的原始与归档 SHA-256（两两相同），且与独立复核证据 `docs/reviews/evidence/phase53-remediation-rereview-2026-07-26/offline-gates-and-evidence-integrity.txt` 的 "Raw log integrity inventory" 12 项逐一相同（同源闭环）。本文 §3 的 5 处日志引用已改为 `.log.txt` 可跟踪路径。
2. **Low（证据凭据脱敏）**：`18b`/`21` 两处（正式复核点名）以及同类主动补充的 `22`，其临时口令、`X-Amz-Credential` 与 `X-Amz-Signature` 值均已替换为 `[REDACTED_SECRET]`；各文件头部注记脱敏前文件 SHA-256（18b=`684769ce…`、21=`f739594f…`、22=`70cfef37…`）。原值仅曾属已销毁的隔离 schema/bucket 且 URL 已自然过期，未在任何其它环境复用，无需轮换。
3. **tracked-path 一次性检查**：`24-tracked-path-check.txt` 归档四段检查结果——本文引用日志路径逐项 `git ls-files --error-unmatch`、证据目录全部非 `.log` 文件跟踪状态、脱敏后 secret lint 复扫零命中、以及 12 个归档日志的暂存 blob SHA-256 与原始 `.log`（即独立复核清单值）逐一 MATCH。
4. 另一 Low（packaged MP4 真实探测自动化门禁）属测试代码变更，按正式报告 §15 划入 "Fix Before Stable Release"，不在本次"仅证据归档/脱敏"整改范围内。

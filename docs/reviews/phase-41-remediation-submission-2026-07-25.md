...
> **2026-07-25 勘误 / 已被后续候选替代：** 本文第 59 行原称特性分支可直接从
> Actions → Run workflow 执行 MySQL 8.4 门禁，该说法不成立；`workflow_dispatch`
> 只有在 workflow 文件已进入默认分支后才可用。合并前只能由用户在获授权的本地
> Linux/WSL 隔离环境执行仓库脚本。当前第二轮材料见
> `phase-41-second-remediation-submission-2026-07-25.md`；本文其余内容只保留为历史快照。

...
## 0. 当前接力快照（2026-07-27）
- 当前分支 `codex/phase00-remediation`；Phase 44 第四轮独立 PASS，U-003 全部关闭。Phase 0 / U-004 第八轮正式独立增量复核为 **CHANGES_REQUESTED（0 Critical / 0 High / 1 Medium / 0 Low）**，报告为 `docs/reviews/phase-00-eighth-remediation-rereview-2026-07-27.md`。最终 HEAD `1c9d416` 中 L1 的 POSIX 完整祖先 fd 链已在代码/合同层面关闭；M1 仍可在 final capture 成功后、canonical marker rename 前改变 artifact，纯离线反例得到 `verified=true` + canonical PASS + checksum mismatch。Windows Python **75 PASS / 2 POSIX-only skipped**、Surefire **274/274**、后端 package **9/9**、Checkstyle 0；未找到 r8 动态 evidence。下一轮只修 M1，固化新最终 SHA 后再补 Linux 77/77、全新隔离栈 exact 7/33、Compose 与资源清理；浏览器不要求。`main` 仍为 `e4f8228`，无 remote，禁止 push/擅自 merge。
...
## 5. 当前整改入口 → Phase 0 / U-004 第九轮只收口 M1
1. 先读 `AGENTS.md` → `docs/REVIEW-GATE.md` → `docs/CURRENT-EXECUTION-PLAN.md` → `docs/reviews/phase-00-eighth-remediation-rereview-2026-07-27.md` → `docs/phase-00-脚手架.md`。第八轮最终 HEAD 为 `1c9d416`；L1 已关闭，不再改 POSIX 设计。下一动作只修 M1 的 final-capture → canonical-marker rename 窗口并补 mutate/delete 反例；不要继续叠加“校验后再校验”状态。固化新最终 clean SHA 后，才由 Linux/POSIX 环境执行 77/77，再由用户/获授权环境重跑完整 gate、Compose config 并报告专属资源销毁。不要求浏览器。Codex 不执行 Docker、数据库、网络、浏览器或其它可能属于 cyber 的指令；Phase 0 正式独立 PASS 后才执行最终全量审计。

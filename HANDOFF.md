...
## 0. 当前接力快照（2026-07-25）
- 当前分支 `codex/phase41-second-remediation`；Phase 39 第二轮代码冻结点 `73406ed` 已由 `docs/reviews/phase-39-second-remediation-rereview-2026-07-24.md` 独立判定 **PASS**。Phase 41 上一轮代码点 `c69ea73` 的正式报告仍为 **CHANGES_REQUESTED（1 High / 2 Medium / 1 Low）**；第二轮候选已冻结到 `b5ed7f5`，待用户动态门禁和新独立报告。`main` 仍为 `e4f8228`，无 remote，禁止 push/擅自 merge。
...
- Phase 41 正式报告仍为 `docs/reviews/phase-41-remediation-rereview-2026-07-25.md`，结论 **CHANGES_REQUESTED（1 High / 2 Medium / 1 Low）**；第二轮提交材料为 `docs/reviews/phase-41-second-remediation-submission-2026-07-25.md`。内部三路只读静态复核 0/0/0、普通安全门禁通过，但真实 MySQL 8.4 CLI 恢复与账号/授权回环均未运行。两项均可能涉及 schema、对象、账号、容器或匿名卷，只能由用户在专用隔离环境执行；Phase 47 不放行。
...
## 5. 当前整改入口 → Phase 41 第二轮动态门禁与独立重核
...
19. 下一步只能由用户在授权 Linux/WSL 专用隔离环境先执行 `scripts/test-phase41-backup-restore-real.sh`（创建/删除两个 scratch schema 与专用桶内一个对象），再执行 `scripts/test-phase41-mysql-init-real.sh`（创建账号并删除两个一次性容器/匿名卷），分别保留 `[phase41-backup-restore-real] PASS` 与 `[phase41-mysql-init-real] PASS`。Codex 不执行这两项可能属于 cyber 的命令；两项 PASS 后仍须独立增量复核，正式 PASS 前 Phase 41 保持复核退回、Phase 47 不放行。

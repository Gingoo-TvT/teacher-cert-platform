...
## 0. 当前接力快照（2026-07-27）
- 当前分支 `codex/phase00-remediation`；Phase 44 第四轮独立 PASS，U-003 全部关闭。Phase 0 / U-004 首轮正式报告 `docs/reviews/phase-00-remediation-rereview-2026-07-27.md` 仍为 **CHANGES_REQUESTED（0 Critical / 0 High / 3 Medium / 4 Low）**；第二轮产品/测试/门禁候选已冻结为 `3bfe83d9eaa4486bb39cc89fc3f05a975319e3af`，提交材料见 `docs/reviews/phase-00-second-remediation-submission-2026-07-27.md`。通用摘要秒传合同已按 R10 退役，CI 锁定 exact 6 suite / 25 testcase 与完整 provenance，权威基线、prod docs 404、参数矩阵及真实 Mapper/DataScope 组合链均已整改。离线 gate 自测 21/21、当前源码 Surefire 266/266、9 模块 package、前端 lint/type-check/build 全绿；真实依赖 exact gate 未由 Codex 执行，阶段与全项目仍未 PASS。`main` 仍为 `e4f8228`，无 remote，禁止 push/擅自 merge。
...
1. 先读 `AGENTS.md` → `docs/REVIEW-GATE.md` → `docs/CURRENT-EXECUTION-PLAN.md` → `docs/reviews/phase-00-remediation-rereview-2026-07-27.md` → `docs/reviews/phase-00-second-remediation-submission-2026-07-27.md` → `docs/phase-00-脚手架.md`。第二轮产品/测试/门禁候选 `3bfe83d` 已完成离线整改；下一动作由用户在获授权的全新一次性 MySQL/Redis/MinIO 隔离栈执行 SHA 绑定的 exact 6/25 gate 并交回完整证据目录，再由独立复核者裁定。Codex 不执行 Docker、数据库、网络、浏览器或其它可能属于 cyber 的指令；第二轮正式 PASS 后才执行最终全量审计。

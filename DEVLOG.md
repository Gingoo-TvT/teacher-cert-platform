...
## [2026-07-04] Phase 43.3（Opus 4.8 执行，Phase 43.3，待复核合并）— §7.5 Rule 11 过程性材料合格永久卡死修复（合格只看最新/有效材料）
...
## [2026-07-04] Phase 43.1（Opus 4.8 执行，Phase 43.1，待复核合并）— P0-12 收尾：不加 DB 外键（决策+理由）+ deleteMajor 改「停用」
...
## [2026-07-04] Phase 41.2（Opus 4.8 执行，Phase 41.2，待复核合并）— P0-6 真备份（替换假/空备份）
...
- 与规格的偏差/疑问：范围严格限 41.2（P0-6），未碰 41.1/41.3（Phase 41 前序已「待复核合并」）或其它 phase。迁移领 **V25**（开工时磁盘 max=V24，`git log` 确认 42.1 已占 V24）。备份类型 `mysql/minio/full` 当前统一执行「逻辑数据导出」（字段用于分类）；物理全备/binlog PITR/MinIO mirror 仍属运维层（手册 §3–§6 保留并去 dev 口令）——**应用内逻辑备份不替代运维 PITR，二者互补**，已在手册顶部声明。
...
## [2026-07-04] Phase 43.2（Opus 4.8 执行，Phase 43.2，待复核合并）— §7.4 证书导出→导入往返/序列/签发日期正确性
...
## [2026-07-04] Phase 43.4（Sonnet 5 执行，Phase 43.4，待复核合并）— 校验 fail-open→fail-closed + 字典编码软删盲修复
...
## [2026-07-04] Phase 42.3（Opus 4.8 执行，Phase 42.3，待复核合并）— 视频计票/幂等并发（settle 丢失更新 + merge 幂等 + thirdReview/returnReview 状态守卫）
...
- 下一步：本 phase 停在分支 `feature/phase42-video` 单个 commit，**未合并入 main**，待人工复核。至此 Phase 37b 遗留的 P0-10 视频计票/幂等子批全部闭环；剩余 P0-10/§7.1 系统性项（`BaseEntity` `@Version` 兜底、其余非视频流转）与 42.1/42.2/42.4 一并待复核合并。

## [2026-07-04] Phase 42.2（Opus 4.8 执行，Phase 42.2，待复核合并）— confirmImport 双确认重复导入（开头原子认领 PREVALIDATED→IMPORTING）
...
## [2026-07-04] Phase 42.1（Opus 4.8 执行，Phase 42.1，待复核合并）— 唯一约束防并发重复创建（迁移 V24：MySQL 生成列 + 活跃态才非空）
...
## [2026-07-04] Phase 42.4（Opus 4.8 执行，Phase 42.4，待复核合并）— 证书 reissue REISSUED 落库 + 原子重开守卫（并 §7.4 死枚举）

...
- 阻塞项：无
- 最近更新：2026-06-17（Phase 10 待复核：V17 导入导出与预校验落地，AT-01 文本兼容 / AT-02 26列+H表头 / AT-14 13条V反例自测通过，`mvn verify` 44/44，前端 type-check/build 通过）
- 下一步：按 `docs/REVIEW-GATE.md` 复核 Phase 10（重点 AT-01/AT-02/AT-14、批次回滚冲突、数据范围与敏感导出鉴权）
...
|---|---|---|---|---|
| AT-01 | 文本字段导入导出一致 | P3/P10 | [~] P3/P8 采集侧已复核通过；P10 导入导出复验自测通过（String模型 + 单元格文本 `@` + 读取按 String，证件号/前导零学号/日期/编号不丢格式），待复核 | [ ] |
| AT-02 | 26 列 + H 表头 | P10 | [~] P10 自测通过：模板/标准导出 A-Z 26列逐列一致，H列=`身份证件号码`，待复核 | [ ] |
...
| AT-14 | 异常报告 | P10 | [~] P10 自测通过：V-01~V-13 各一反例命中并定位行/字段/原因，异常行不入库，异常报告字段齐全，待复核 | [ ] |

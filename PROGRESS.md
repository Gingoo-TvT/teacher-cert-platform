...
## 当前焦点
- 当前阶段：**Phase 9 证书（T-072~T-079）待复核**；Phase 0~8 已复核通过。
- 阻塞项：无
- 最近更新：2026-06-17（Phase 9 待复核：证书编号/有效期/前置聚合/状态机C/作废重开/更正/前端页面完成；`mvn verify` 40/40，前端 type-check/build 通过。）
- 下一步：按 `docs/REVIEW-GATE.md` 复核 Phase 9（AT-09 前置条件 / AT-10 18位编号连续不重 / AT-11 有效期）
...
| AT-09 | 证书前置条件 | P9 | [~] P9自测通过：缺过程性考核生成拒绝，缺失清单含“过程性考核”；复核后置 ✅ | [ ] |
| AT-10 | 18位编号连续不重 | P9 | [~] P9自测通过：18位分段示例可复现，SCHOOL_YEAR/SCHOOL_YEAR_SEGMENT 参数生效，50并发序号连续不重无空号；复核后置 ✅ | [ ] |
| AT-11 | 有效期规则 | P9 | [~] P9自测通过：上半年签发→+3年6/30，下半年签发→+3年12/31；复核后置 ✅ | [ ] |
| AT-12 | 审核全留痕 | P13 | [ ] | [ ] |

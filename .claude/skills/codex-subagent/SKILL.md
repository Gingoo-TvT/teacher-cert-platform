---
name: codex-subagent
description: Drive the codex CLI headlessly as an implementation subagent that Claude schedules and monitors, then gate-reviews per docs/REVIEW-GATE.md. Use when the user wants codex to do the coding/phase work on teacher-cert-platform while Claude orchestrates, watches progress live, and reviews before release. Triggers — "用codex/让codex做", "codex作为subagent", "codex实现这个阶段/任务", "调度并监控codex".
---

# codex-subagent — 用 codex 作为受 Claude 调度的实现子代理

Claude 负责 **调度 + 监控 + 复核放行**；codex（`codex exec`，gpt-5.5/xhigh）负责 **实现**。闭环：Claude 派活 → codex 写代码并自测、置「待复核」→ Claude 监控全程 → 按 `docs/REVIEW-GATE.md` 复核 → PASS 放行 / 退回。
...
- **工作树独占**：codex 跑时 Claude 不并发改文件、不占用 8080（codex 可能 `mvn package` + 起应用自测）。先把 Claude 自己启动的 app 关掉（`taskkill //F //IM java.exe`）。
...
- 一次只让一个写工作树；codex 跑时 Claude 只读不写。
